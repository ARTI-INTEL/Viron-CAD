...
// ── Helper: generic PDF fallback (types without a template) ────
function generateGenericPdf(doc, report, officerName, serverName) {
  ...
  doc.fontSize(8).fillColor('#999').text(
    `Generated by Viron CAD on ${new Date().toLocaleString()}`,
...
router.get('/:id/pdf', verifyUser, async (req, res) => {
  try {
    ...
    const serverName = serverRows[0]?.name || 'Viron CAD';
    ...
    // ── Look up officer's unit department to help resolve ambiguous templates ──
    let officerDeptType = '';
    if (report.officer_id) {
      try {
        const [unitRows] = await pool.query(
          `SELECT u.department FROM units u WHERE u.user_id = ? AND u.server_id = ? LIMIT 1`,
          [report.officer_id, report.server_id]
        );
        if (unitRows.length) {
          const dept = (unitRows[0].department || '').toLowerCase();
          if (dept.includes('fire') || dept.includes('rescue')) officerDeptType = 'FR';
          else if (dept.includes('transport') || dept.includes('dot')) officerDeptType = 'DOT';
          else officerDeptType = 'LEO';
        }
      } catch (_) { /* ignore — fallback will work */ }
    }

    // ── Try the template system first ────────────────────────
    let templatePdf = null;
    try {
      templatePdf = await renderReportToTemplate(report, officerName, serverName, {
        officerDeptType,
      });
    } catch (_) {
      // Template render failed (missing/corrupted file, etc.) — fall through to generic
    }

    if (templatePdf) {
      // Template was found and rendered — send it
      res.end(templatePdf);
      return;
    }

    // ── Fallback: generic pdfkit generation ──────────────────
    const doc = new PDFDocument({ margin: 50 });
    doc.pipe(res);
    generateGenericPdf(doc, report, officerName, serverName);
    doc.end();
  } catch (err) {
    logError(err);
