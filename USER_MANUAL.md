...
**Viron CAD** is a web-based Computer-Aided Dispatch system built for Roblox roleplay communities. It provides a complete suite of tools for emergency services roleplay, including multi-agency dispatch, incident reporting, record keeping, department management, and real-time integration with **Emergency Response: Liberty County (ERLC)**.
...
1. Navigate to your Viron CAD instance in your browser.
...
Viron CAD integrates with **Emergency Response: Liberty County (ERLC)** to provide real-time game data within the CAD interface.
...
2. The recording stops, and the video file is saved to `~/VironCAD/Bodycam/`.
...
| `/server-link` | Check if this Discord server is linked to an Viron CAD server | 👤 Ephemeral |
...
Viron CAD includes an **Alert Sounds** system that plays audio notifications for important events. Sounds are generated by the Web Audio API by default, with support for custom audio files.
...
Viron CAD uses Discord OAuth — there is no separate password. Simply click **"Login with Discord"** and re-authorize. If your Discord account is compromised, secure it first through Discord.
...
*This manual was generated for **Viron CAD**. For additional support or feature requests, please contact your server administrator or visit the documentation site.*

*© 2026 Viron CAD — Built for the Roblox roleplay community.*
